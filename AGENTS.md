...
`mock/vite-mock-api.js` (Vite-плагин, `apply: "serve"`, только dev): `POST /user/ad`, `POST /user/lk`, `GET /user/phonedir`. Единственный источник токена LiveKit — `POST /user/lk` (своя комната — `num` = `room` = свой номер, приглашение — `num` приглашаемого и `room` своей комнаты). Токен LiveKit подписывается dev-ключом.
