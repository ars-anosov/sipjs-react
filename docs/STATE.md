...
Владение рендером — по срезу: `AuthContainer` (домен `authControlRdcr`) держит оба AD-блока — форму `AuthAd` (флаг `displayAd` или `errComponent === "AuthAd"`) и `AuthPad`; `PhoneContainer` (домен `phoneControlRdcr`) рендерит форму входа `PhoneReg` (флаг `displayReg` или `errComponent === "PhoneReg"`) и рабочие блоки `PhonePad` / `PhoneHistory` / `PhoneChat`, auth-срез не читает. Обе формы входа — модальный `Dialog` в портале, вне потока документа, поэтому их показ не раздвигает вёрстку; чужие срезы читает лишь `AuthContainer` — как мост.
