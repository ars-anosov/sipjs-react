...
Стартовый экран: пока `authControlRdcr.status !== "success"` и `phoneControlRdcr.regState !== "ok"`, виден только блок `AuthLinks` (иконка-вход в SIP-форму; сам ничего не диспатчит — колбэк `AuthContainer` пишет в `phoneControlRdcr`). Успех AD → `AuthPad`, успешная SIP-регистрация → телефон (`displayPad` на `PHONECTL_CONNECT_SUCCESS`); заголовок `App` — flex-колонка на высоту экрана, поэтому стартовый блок центрируется между AppBar и футером.
