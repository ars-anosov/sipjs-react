...
sequenceDiagram
  ...
  Note over User,Dispatch: Старт: displayAd=false, displayAuthPad=false, displayReg=false, regState=off
  Note over User,Dispatch: на экране только ссылки AuthLinks — входы в обе формы
