...
- `LkMeet` берёт `lk_room`/`lk_token` из query маршрута (HashRouter, вид `#/?lk_room=...`); без
  AD/`lk_token` показывает информирующий текст, тумблер заблокирован. Готовый токен, своя комната
  и identity, порядок шагов и грабли (`.lk-control-bar` не доказывает подключение; комнату `9994`
  занимает браузер разработчика) — `docs/LIVEKIT.md`.
